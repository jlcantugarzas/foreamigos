...
export default function App() {
  ...
  if(!ready)return(<div style={{...S.app,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,minHeight:"100vh"}}><div style={{fontSize:56}}>â³</div><div style={{color:"#e8c84a",fontWeight:700,fontSize:22}}>ForeAmigos</div><div style={S.muted}>Conectando...</div><style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style><div style={{width:36,height:36,border:"3px solid #2a4030",borderTopColor:"#e8c84a",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/></div>);
  ...
  if(screen==="home"&&currentPlayer){
    ...
    return(<div style={S.app}><Notification data={notification}/>{syncing&&<SyncBadge/>}<div style={{padding:"1.5rem 1rem 5rem"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.5rem"}}><div><div style={S.h1}>{course.name}</div><div style={{...S.muted,display:"flex",alignItems:"center",gap:6}}><span style={{width:7,height:7,borderRadius:"50%",background:"#4ade80",display:"inline-block"}}/>Live</div></div><button onClick={()=>{setCurrentPlayer(null);setScreen("login");}} style={S.btnSm("#2a4030","#7a9080")}>Logout</button></div><div style={{...S.cardGold,marginBottom:"1rem",display:"flex",gap:12,alignItems:"center"}}><Avatar player={currentPlayer} size={52}/><div><div style={{fontSize:20,fontWeight:700,color:"#e8c84a"}}>Â¡Bienvenido, {currentPlayer.name}!</div><div style={{color:"#8fa898",fontSize:14}}>{team?.name} Â· HCP {currentPlayer.handicap}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:"1rem"}}><StatCard label="Stableford" value={stats.stableford} unit="pts"/><StatCard label="Quota" value={(stats.quotaPos>=0?"+":"")+stats.quotaPos} unit={"de "+stats.quota}/><StatCard label="Hoyos" value={stats.holesPlayed+"/"+course.holes} unit="jugados"/><StatCard label="Gross" value={stats.holesPlayed>0?stats.totalGross:"â"} unit="golpes"/></div><button onClick={()=>setScreen("scorecard")} style={{...S.btn(),marginBottom:"1rem",fontSize:17,padding:16}}>â³ Ingresar Scores</button><button onClick={()=>setScreen("leaderboard")} style={{...S.btn("#1a2e1e","#e8c84a"),marginBottom:"1rem",border:"1px solid #b8962e"}}>ð Leaderboard en Vivo</button><div style={{...S.card,marginBottom:"1rem"}}><div style={{...S.h3,marginBottom:12}}>Premios Especiales</div><SpecialAwards players={players} closestPin={closestPin} longestDrive={longestDrive} setClosestPin={cp=>{setClosestPin(cp);updateTournament({closestPin:cp});}} setLongestDrive={ld=>{setLongestDrive(ld);updateTournament({longestDrive:ld});}} currentPlayer={currentPlayer} course={course} showNotif={showNotif}/></div></div><BottomNav screen="home" setScreen={setScreen}/></div>);
  }
  ...
  if(screen==="leaderboard")return(<div style={S.app}><Notification data={notification}/>{syncing&&<SyncBadge/>}<div style={{padding:"1.5rem 1rem 5rem"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.5rem"}}><div style={S.h1}>Leaderboard</div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{width:8,height:8,borderRadius:"50%",background:"#4ade80",display:"inline-block"}}/><span style={{...S.muted,fontSize:11}}>LIVE</span><button onClick={()=>setScreen("home")} style={S.btnSm("#2a4030","#7a9080")}>AtrÃ¡s</button></div></div><div style={{display:"flex",gap:8,background:"#0f1f14",borderRadius:12,padding:4,marginBottom:"1rem"}}>{["individual","teams","scorecard"].map(t=>(<button key={t} style={S.tab(activeTab===t)} onClick={()=>setActiveTab(t)}>{t==="individual"?"Individual":t==="teams"?"Equipos":"Scorecard"}</button>))}</div>{activeTab==="individual"&&<IndividualLeaderboard players={players} scores={scores} course={course} teams={teams}/>}{activeTab==="teams"&&<TeamLeaderboard teams={teams} players={players} scores={scores} course={course}/>}{activeTab==="scorecard"&&<FullScorecard players={players} scores={scores} course={course} currentPlayer={currentPlayer}/>}</div><BottomNav screen="leaderboard" setScreen={setScreen}/></div>);
...
function IndividualLeaderboard({players,scores,course,teams}){
  ...
  return(<div>{ranked.map((p,i)=>(<div key={p.id} style={{...S.card,marginBottom:8,display:"flex",alignItems:"center",gap:12,borderLeft:i===0?"3px solid #e8c84a":"1px solid #2a4030"}}><div style={{fontSize:20,fontWeight:800,color:i===0?"#e8c84a":i===1?"#c0c0c0":i===2?"#cd7f32":"#4a6050",minWidth:28,textAlign:"center"}}>{i===0?"ð¥":i===1?"ð¥":i===2?"ð¥":i+1}</div><Avatar player={p} size={38}/><div style={{flex:1}}><div style={{fontWeight:600}}>{p.name}</div><div style={S.muted}>{p.team?.name} Â· HCP {p.handicap}</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:800,fontSize:18,color:p.quotaPos>0?"#4ade80":p.quotaPos<0?"#f87171":"#f0ede4"}}>{p.holesPlayed>0?(p.quotaPos>=0?"+":"")+p.quotaPos:"â"}</div><div style={S.muted}>{p.stableford}pts Â· {p.holesPlayed}H</div></div></div>))}<div style={{...S.muted,textAlign:"center",marginTop:8,fontSize:12}}>Ordenado por cuota</div></div>);
}
...
function SpecialAwards({players,closestPin,longestDrive,setClosestPin,setLongestDrive,currentPlayer,course,showNotif}){
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{background:"#0f1f14",borderRadius:10,padding:12}}><div style={{fontWeight:600,color:"#e8c84a",marginBottom:8}}>ð MÃ¡s Cerca</div><select value={cpH} onChange={e=>setCpH(parseInt(e.target.value))} style={{...S.input,width:"auto",marginBottom:8}}>{Array.from({length:course.holes},(_,i)=><option key={i} value={i+1}>Hoyo {i+1}</option>)}</select>{cp?<div style={{...S.muted,marginBottom:6}}>Actual: <span style={{color:"#4ade80",fontWeight:600}}>{cp.name}</span></div>:<div style={{...S.muted,marginBottom:6}}>Sin reclamar</div>}<button onClick={()=>{const c={...closestPin,[cpH]:currentPlayer.id};setClosestPin(c);showNotif(currentPlayer.name+" reclama H"+cpH+"! ð");}} style={S.btnSm()}>Reclamar H{cpH}</button></div><div style={{background:"#0f1f14",borderRadius:10,padding:12}}><div style={{fontWeight:600,color:"#e8c84a",marginBottom:8}}>ð¨ Drive MÃ¡s Largo</div>{ld?<div style={{...S.muted,marginBottom:6}}>Actual: <span style={{color:"#4ade80",fontWeight:600}}>{ld.name}</span></div>:<div style={{...S.muted,marginBottom:6}}>Sin reclamar</div>}<button onClick={()=>{setLongestDrive(currentPlayer.id);showNotif(currentPlayer.name+" reclama LD! ð¨");}} style={S.btnSm()}>Reclamar</button></div></div>);
}
...
function BottomNav({screen,setScreen}){return(<div style={{position:"fixed",bottom:0,left:0,right:0,background:"#0a1a0d",borderTop:"1px solid #2a4030",display:"flex",padding:"8px 0 12px",zIndex:100}}>{[{id:"home",icon:"ð ",label:"Inicio"},{id:"scorecard",icon:"â³",label:"Score"},{id:"leaderboard",icon:"ð",label:"Board"}].map(it=>(<button key={it.id} onClick={()=>setScreen(it.id)} style={{flex:1,background:"none",border:"none",color:screen===it.id?"#e8c84a":"#4a6050",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><div style={{fontSize:20}}>{it.icon}</div><div style={{fontSize:11,fontWeight:600}}>{it.label}</div></button>))}</div>);}
