...
function calcStableford(gross,par,strokes) {
  ...
  if(g>=getPickupScore(par,strokes))return 0;
...
function getTotalStats(player,scores,course,allPlayers) {
  ...
  const quota=Math.round((course.holes/18)*(36-playingHcp(player.handicap)));
...
export default function App() {
  ...
  const notifPlayingRef=useRef(false);
  const notifTimerRef=useRef(null);
  const flushNotifRef=useRef(null);
  flushNotifRef.current=()=>{if(notifPlayingRef.current||!notifQueueRef.current.length)return;notifPlayingRef.current=true;const next=notifQueueRef.current.shift();setNotification(next);notifTimerRef.current=setTimeout(()=>{setNotification(null);notifPlayingRef.current=false;flushNotifRef.current();},2000);};
  const pushNotif=useCallback((msg,type="score")=>{notifQueueRef.current.push({msg,type});flushNotifRef.current();},[]);
  const dismissNotif=useCallback(()=>{clearTimeout(notifTimerRef.current);setNotification(null);notifPlayingRef.current=false;setTimeout(()=>flushNotifRef.current(),50);},[]);

  const showNotif=(msg,type="success")=>pushNotif(msg,type);
    ...
    const u1=onSnapshot(doc(db,"tournament","current"),snap=>{
      if(snap.exists()){const d=snap.data();if(d.players)setPlayers(d.players);if(d.teams)setTeams(d.teams);if(d.course)setCourse(d.course);if(d.closestPin!==undefined)setClosestPin(d.closestPin||{});if(d.longestDrive!==undefined)setLongestDrive(d.longestDrive);
        ...
        if(d.lastPickup&&d.lastPickup.ts&&d.lastPickup.ts!==window._lastPickupTs){
          ...
          if(!window._myScoreTs?.has(d.lastPickup.ts)){
            ...
            pushNotif(msg,isPickupAlert?"pickup":"score");
          }
          window._myScoreTs?.delete(d.lastPickup.ts);
        }}
    });
...
function ScorecardScreen({currentPlayer,players,course,scores,updateScore,updateTournament,scoringHole,setScoringHole,onBack,showNotif,syncing,onScoreAlert,setScreen,chatMsg,chatOpen,setChatOpen,chatInput,setChatInput,sendChat,chatHistory,notification,onDismiss}){
