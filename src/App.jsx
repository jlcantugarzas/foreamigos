...
export default function App() {
  ...
  if(!ready)return(<div style={{...S.app,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,minHeight:"100vh"}}><div style={{fontSize:56}}>(golf)</div><div style={{color:"#e8c84a",fontWeight:700,fontSize:22}}>ForeAmigos</div><div style={S.muted}>Conectando...</div><style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style><div style={{width:36,height:36,border:"3px solid #2a4030",borderTopColor:"#e8c84a",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/></div>);
...
function TeamLeaderboard({teams,players,scores,course}){
  ...
  return(<div>{ts.map((t,i)=>(<div key={t.id} style={{...S.card,marginBottom:12,borderLeft:"4px solid "+t.color}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}><div><div style={{fontWeight:700,fontSize:17,color:t.color}}>{i===0?"TrophyTrophy ":""}{t.name}</div><div style={S.muted}>{t.holesPlayed} hoyos</div></div><div style={{fontSize:32,fontWeight:800,color:"#e8c84a"}}>{t.totalSF}</div></div>{t.members.map(p=>{const s=getTotalStats(p,scores,course,players);return(<div key={p.id} style={{display:"flex",alignItems:"center",gap:8,paddingTop:8,borderTop:"1px solid #2a4030"}}><Avatar player={p} size={28}/><span style={{fontSize:13,flex:1}}>{p.name}</span><span style={{fontWeight:700,color:"#e8c84a",fontSize:14}}>{s.stableford}pts</span></div>);})}</div>))}</div>);
}
...
function BottomNav({screen,setScreen}){return(<div style={{position:"fixed",bottom:0,left:0,right:0,background:"#0a1a0d",borderTop:"1px solid #2a4030",display:"flex",padding:"8px 0 12px",zIndex:100}}>{[{id:"home",icon:"",label:"Inicio"},{id:"scorecard",icon:"(golf)",label:"Score"},{id:"leaderboard",icon:"TrophyTrophy",label:"Board"}].map(it=>(<button key={it.id} onClick={()=>setScreen(it.id)} style={{flex:1,background:"none",border:"none",color:screen===it.id?"#e8c84a":"#4a6050",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><div style={{fontSize:20}}>{it.icon}</div><div style={{fontSize:11,fontWeight:600}}>{it.label}</div></button>))}</div>);}
