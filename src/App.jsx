...
function BottomNav({screen,setScreen}){return(<div style={{position:"fixed",bottom:0,left:0,right:0,background:"#0a1a0d",borderTop:"1px solid #2a4030",display:"flex",padding:"8px 0 12px",zIndex:100}}>{[{id:"home",icon:"ð ",label:"Inicio"},{id:"scorecard",icon:"â³",label:"Score"},{id:"leaderboard",icon:"🏆",label:"Board"}].map(it=>(<button key={it.id} onClick={()=>setScreen(it.id)} style={{flex:1,background:"none",border:"none",color:screen===it.id?"#e8c84a":"#4a6050",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}><div style={{fontSize:20}}>{it.icon}</div><div style={{fontSize:11,fontWeight:600}}>{it.label}</div></button>))}</div>);}
