...
export default function App() {
    ...
    const u1=onSnapshot(doc(db,"tournament","current"),snap=>{
      if(snap.exists()){const d=snap.data();if(d.players)setPlayers(d.players);if(d.teams)setTeams(d.teams);if(d.course)setCourse(d.course);if(d.closestPin!==undefined)setClosestPin(d.closestPin||{});if(d.longestDrive!==undefined)setLongestDrive(d.longestDrive);
        if(d.lastPickup&&d.lastPickup.ts&&d.lastPickup.ts!==window._lastPickupTs){
          ...
          setNotification({msg:d.lastPickup.playerName+': "'+d.lastPickup.phrase+'"',type:"pickup"});
  ...
  if(screen==="scorecard"&&currentPlayer)return <ScorecardScreen currentPlayer={currentPlayer} players={players} course={course} scores={scores} updateScore={updateScore} updateTournament={updateTournament} scoringHole={scoringHole} setScoringHole={setScoringHole} onBack={()=>setScreen("home")} showNotif={showNotif} syncing={syncing}/>;
...
function ScorecardScreen({currentPlayer,players,course,scores,updateScore,updateTournament,scoringHole,setScoringHole,onBack,showNotif,syncing}){
...
function Notification({data}){if(!data)return null;const isPickup=data.type==="pickup";return(<div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",background:isPickup?"#2a1a00":data.type==="error"?"#4a1a1a":"#1a3d2a",color:isPickup?"#fbbf24":data.type==="error"?"#f87171":"#4ade80",padding:isPickup?"14px 20px":"10px 20px",borderRadius:12,fontWeight:600,fontSize:isPickup?15:14,zIndex:200,border:"1px solid "+(isPickup?"#fbbf24":data.type==="error"?"#f87171":"#4ade80"),maxWidth:"90vw",textAlign:"center",boxShadow:isPickup?"0 4px 20px rgba(251,191,36,0.3)":"none"}}>{isPickup?"<< ":"}"}{data.msg}{isPickup?" >>":""}</div>);}