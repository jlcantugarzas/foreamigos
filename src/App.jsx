...
export default function App() {
  ...
  if(screen==="leaderboard")return(<div style={S.app}><Notification data={notification} onDismiss={()=>setNotification(null)}/>{syncing&&<SyncBadge/>}<div style={{padding:"1.5rem 1rem 5rem"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.5rem"}}><div style={S.h1}>Leaderboard</div><div style={{display:"flex",gap:8,alignItems:"center"}}><span style={{width:8,height:8,borderRadius:"50%",background:"#4ade80",display:"inline-block"}}/><span style={{...S.muted,fontSize:11}}>LIVE</span><button onClick={()=>setScreen("home")} style={S.btnSm("#2a4030","#7a9080")}>Atrs</button></div></div><div style={{display:"flex",gap:8,background:"#0f1f14",borderRadius:12,padding:4,marginBottom:"1rem"}}>{["individual","teams","scorecard"].map(t=>(<button key={t} style={S.tab(activeTab===t)} onClick={()=>setActiveTab(t)}>{t==="individual"?"Individual":t==="teams"?"Equipos":"Scorecard"}</button>))}</div>{activeTab==="individual"&&<IndividualLeaderboard players={players} scores={scores} course={course} teams={teams}/>}{activeTab==="teams"&&<TeamLeaderboard teams={teams} players={players} scores={scores} course={course}/>}{activeTab==="scorecard"&&<FullScorecard players={players} scores={scores} course={course} currentPlayer={currentPlayer}/>}</div><ChatBanner msg={chatMsg}/>
      <ChatInput player={currentPlayer} onSend={sendChat} open={chatOpen} setOpen={setChatOpen} value={chatInput} setValue={setChatInput}/>
      <BottomNav screen="leaderboard" setScreen={setScreen}/></div>);
...
function ScorecardScreen({currentPlayer,players,course,scores,updateScore,updateTournament,scoringHole,setScoringHole,onBack,showNotif,syncing,onScoreAlert,setScreen,chatMsg,chatOpen,setChatOpen,chatInput,setChatInput,sendChat}){
  ...
  return(<div style={S.app}>{syncing&&<SyncBadge/>}<div style={{padding:"1rem 1rem 6rem"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem"}}><button onClick={onBack} style={S.btnSm("#2a4030","#8fa898")}> Atrs</button><div style={{textAlign:"center"}}><div style={{fontWeight:700,color:"#e8c84a"}}>{currentPlayer.name}</div><div style={S.muted}>HCP {currentPlayer.handicap}</div></div><div style={{...S.muted,fontSize:13}}>{course.holes}H</div></div><div style={{display:"flex",gap:8,marginBottom:10}}>
    ...
    <button onClick={()=>{
      ...
      const msg=currentPlayer.name+" recogió su pelota 🏳️‍🌈";
      ...
      updateTournament({lastAlert:{msg,type:"pickup",ts:Date.now()}});
      ...
      {open?(
        <div style={{background:"#1a2e1e",border:"1px solid #2a4030",borderRadius:12,padding:"8px 10px",display:"flex",gap:8,alignItems:"center"}}>
          <input
            ...
            onKeyDown={e=>{if(e.key==="Enter"){onSend(value,player.name);e.preventDefault();}if(e.key==="Escape")setOpen(false);}}
            placeholder="Escribe algo..."
            style={{flex:1,background:"transparent",border:"none",color:"#f0ede4",fontSize:14,outline:"none"}}
          />
          <span style={{fontSize:11,color:"#4a6050"}}>{value.length}/80</span>
          <button onClick={()=>onSend(value,player.name)} style={{background:"#1a6b3c",border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontWeight:600,fontSize:13,cursor:"pointer"}}>Enviar</button>
          <button onClick={()=>setOpen(false)} style={{background:"transparent",border:"none",color:"#4a6050",fontSize:18,cursor:"pointer",padding:"0 4px"}}>x</button>
        </div>
      ):(
        <button onClick={()=>setOpen(true)} style={{width:"100%",background:"#1a2e1e",border:"1px solid #2a4030",borderRadius:12,padding:"8px 14px",color:"#4a6050",fontSize:13,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:"#2a4030"}}/>
          Di algo al grupo...
        </button>
      )}
    </div>
