...
export default function App() {
  ...
  if(screen==="scorecard"&&currentPlayer)return <ScorecardScreen currentPlayer={currentPlayer} players={players} course={course} scores={scores} updateScore={updateScore} updateTournament={updateTournament} scoringHole={scoringHole} setScoringHole={setScoringHole} onBack={()=>setScreen("home")} showNotif={showNotif} syncing={syncing} setScreen={setScreen} setScreen={setScreen} onScoreAlert={(msg)=>{updateTournament({lastPickup:{playerName:'',phrase:msg,hole:0,ts:Date.now()}});setNotification({msg,type:"score"});}} />;
...
function ScorecardScreen({currentPlayer,players,course,scores,updateScore,updateTournament,scoringHole,setScoringHole,onBack,showNotif,syncing,onScoreAlert,setScreen}){
