...
export default function App() {
  ...
  flushNotifRef.current=()=>{if(notifPlayingRef.current||!notifQueueRef.current.length)return;notifPlayingRef.current=true;const next=notifQueueRef.current.shift();setNotification(next);notifTimerRef.current=setTimeout(()=>{setNotification(null);notifPlayingRef.current=false;flushNotifRef.current();},1500);};
  ...
  if(screen==="scorecard"&&currentPlayer)return <ScorecardScreen currentPlayer={currentPlayer} players={players} course={course} scores={scores} updateScore={updateScore} updateTournament={updateTournament} scoringHole={scoringHole} setScoringHole={setScoringHole} onBack={()=>setScreen("home")} showNotif={showNotif} syncing={syncing} setScreen={setScreen} chatMsg={chatMsg} chatOpen={chatOpen} setChatOpen={setChatOpen} chatInput={chatInput} setChatInput={setChatInput} sendChat={sendChat} chatHistory={chatHistory} notification={notification} onDismiss={dismissNotif} onScoreAlert={(msg)=>{updateTournament({lastPickup:{playerName:'',phrase:msg,hole:0,ts:Date.now()}});}} />;
